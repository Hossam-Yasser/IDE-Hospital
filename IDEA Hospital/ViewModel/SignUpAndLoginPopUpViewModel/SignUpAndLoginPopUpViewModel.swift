...
extension SignUpAndLoginPopUpViewModel{
    private func dataValidation(name: String , email: String , mobile: String, password: String, voucher: String, patientName: String) ->Bool {
        ...
        if !ValidationManager.shared().isValidEmail(email: email){
            self.view.showCusromAlert(type: .defaultAlert(title: "Sorry"), message: "Please Enter Valid Email as: abc@efg.com", title: "OK")
            return false
        }
        
        if !ValidationManager.shared().isEmptyMobile(mobile: mobile){
            self.view.showCusromAlert(type: .defaultAlert(title: "Sorry"), message: "Please Enter Valid Mobile Number Of 11 Numbers", title: "OK")
            return false
        }
        
        if !ValidationManager.shared().isValidPassword(password: password){
            self.view.showCusromAlert(type: .defaultAlert(title: "Sorry"), message: "Please Enter Valid Password: at Least 6 Characters Number, Capital and Small Letters", title: "OK")
            return false
        }
